import { makeTemplates } from "./index";

describe("Snapshots", () => {
  it("Test buggy document", async () => {
    const file = {
      id: "buggy",
      html,
      css
    };
    const result = await makeTemplates(file);
    const cssResult = result.metaTemplates.find(
      item => item.formatId === "css"
    );
    const cssFiles = JSON.stringify(cssResult.files);

    const hasClassName = cssFiles.includes("input__item");
